...
export default function DashboardLayout({
  ...
  const { loading, isAuthenticated, user } = useAuth({ redirectOnUnauthenticated: true, redirectPath: '/login' });
  ...
  if (!isAuthenticated) {
    // O hook useAuth cuida de redirecionar para /login automaticamente
    return <DashboardLayoutSkeleton />
  }
  ...
  return (
    <>
      <div className="relative" ref={sidebarRef}>
        ...
        >
          <SidebarHeader className="h-16 justify-center admin-sidebar-header">
          ...
          <SidebarContent className="gap-0">
            {menuItems.map((section, sectionIdx) => (
              <div key={section.section} className={sectionIdx > 0 ? "mt-2" : ""}>
                {!isCollapsed && (
                    ...
                    return (
                      <SidebarMenuItem key={item.path}>
                        <SidebarMenuButton
                          ...
                          tooltip={item.label}
                          className={`h-10 transition-all font-normal ${isActive ? "admin-menu-active" : ""}`}
                        >
                          <item.icon
                            className={`h-4 w-4 ${
